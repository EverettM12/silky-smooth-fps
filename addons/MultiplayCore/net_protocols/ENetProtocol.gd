...
func host(port, bind_ip, max_players) -> MultiplayerPeer:
	...
	# FIX: Only apply config if creation succeeded
	if err == OK:
		_apply_peer_config(peer, bind_ip)
	else:
		MPIO.logerr("Server host failed. Error Code: " + str(err))
...
func join(address, port) -> MultiplayerPeer:
	...
	if err == OK:
		...
		# This will print WHY it failed in your Output console
		MPIO.logerr("Client connect failed. Error Code: " + str(err))
